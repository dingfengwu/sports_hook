<template>
	<view class="add-bank-card-sub-container zh">
		<uni-forms :modelValue="form" ref="form" :rules="rules">
			...
			<uni-forms-item name="ifsc" :label="$t('withdraw.ifsc')" class="ifsc">
				<uni-easyinput :inputBorder="false" trim="both" confirmType="next" type="text" v-model="form.ifsc"
					:placeholder="$t('withdraw.messageIFSC')" />
			</uni-forms-item>
		...
		<uni-popup ref="successPopup" type="message">
			<uni-popup-message type="success" :message="$t('withdraw.messageAddBankCardSuccess')" :duration="2000"></uni-popup-message>
		</uni-popup>
	</view>
</template>
					...
					ifsc: {
						rules: [{
							...
							errorMessage: this.$t("common.pleaseInputField", {
								field: this.$t("withdraw.ifsc")
							}),
						}, {
							validateFunction: function(rule, value, data, callback) {
								console.log(value, rule)
								if (!/^[A-Z]{4}0[1-9]{6}$/.test(value)) {
									let message = _this.$t("withdraw.messageIFSC");
									callback(message);
								}
								return true;
							}
