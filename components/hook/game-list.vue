<template>
	<view class="record-list game-list">
		<scroll-view-infinity-load @refresh="refreshData" @load="loadData" :load-completed="loadCompleted" :show-completed="showCompleted">
			<hook-item v-for="(item, index) in matchList" :key="index" :item="item" v-if="item.status !== 2" :openable="false"></hook-item>
...
import { MATCH_STATUS_BETABLE } from '../../common/js/util/const.js';
import { addDays, formatDate, getCurrentTime, getCurrentBeijinTime } from '../../common/js/util/util.js';
